window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".rt-badge { font-size: 11px; padding: 2px 8px; border-radius: 999px; border: 1px solid var(--dsw-alias-border-l2); color: var(--dsw-alias-label-secondary); }",
			...
			".rt-num { width: 84px; }",
			".rt-btn { border: none; border-radius: 8px; padding: 5px 14px; cursor: pointer; background: var(--dsw-alias-brand-primary); color: #fff; font-size: 13px; }",
				...
				children.push(React.createElement("div", { className: "rt-row", key: row.provider },
					...
					row.configurable && row.policy !== null
						? React.createElement("div", { className: "rt-body" },
							React.createElement("select", {
								...
								React.createElement("option", { value: "normal" }, "普通(达到次数上限后停止)"),
								React.createElement("option", { value: "always" }, "始终重试(不设上限)")),
							draft.mode === "normal"
								? React.createElement("label", { className: "rt-meta" },
									"失败重试次数",
								...
								: React.createElement("span", { className: "rt-meta" }, "所有失败都会按指数退避无限重试,直到成功或取消"),
					...
					row.policy !== null && bo !== null
						? React.createElement("p", { className: "rt-meta" },
							...
							+ (row.policy.retryableCodes !== undefined ? ";可重试错误:" + row.policy.retryableCodes.join(" / ") : ""))
						: null,
					!row.configurable
