window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".rt-select, .rt-num { background: var(--dsw-alias-bg-layer-1); color: var(--dsw-alias-label-primary); border: 1px solid var(--dsw-alias-border-l2); border-radius: 8px; padding: 4px 8px; font-size: 13px; }",
		...
		function Section(props) {
			...
			function save(row) {
				...
				api.settings.mutate({
					...
					setMessage({ kind: "ok", text: "已保存 " + row.displayName + ":对它的下一次模型请求生效(其他行未保存的修改不受影响)" });
					reload(row.provider);
				}, function (err) {
					setBusy(null);
			...
			function reset(row) {
				...
				api.settings.mutate({
					...
					setMessage({ kind: "ok", text: "已恢复 " + row.displayName + " 的默认策略(普通模式,最多重试 " + DEFAULT_MAX_RETRIES + " 次)" });
					reload(row.provider);
				}, function (err) {
					setBusy(null);
